...
import { arrayToDictionary } from '@uap/utils';
...
export class ToppingsEffects {
    ...
    public loadToppings$: Observable<ToppingsLoaded> = this.dataPersistence.fetch(
        ToppingsActionTypes.ToppingsLoad,
        {
            run: (action: ToppingsLoad, state: ProductsPartialState) => {
                if (state[PRODUCTS_FEATURE_KEY].toppings.loaded) {
                    return new ToppingsLoaded(state[PRODUCTS_FEATURE_KEY].toppings.data);
                }
                return new ToppingsLoaded(
                    arrayToDictionary<Topping>([
                        {
                            id: 1,
                            name: 'anchovy',
                        },
